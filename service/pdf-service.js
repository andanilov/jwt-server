...
class PdfService {
  async addTableFile(tpl, fileBuffer) {
    ...
    // 5. Update Db table struct
    // Object.entries(tableMap).forEach(([key, key]))
    Object.keys(tableMap).forEach(async (col) =>
      await db.query(`ALTER TABLE pdfdata ADD COLUMN IF NOT EXISTS ${col} VARCHAR(${maxDataLen})`)
    );
